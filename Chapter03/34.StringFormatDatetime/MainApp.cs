...
namespace StringFormatDatetime
{
    class MainApp
    {
        static void Main(string[] args)
        {
            ...
            WriteLine();
            WriteLine(dt.ToString("0:yyyy-MM-dd tt hh:mm:ss (ddd)", ciKo));
            WriteLine(dt.ToString("0:yyyy-MM-dd HH:mm:ss (dddd)", ciKo));
            WriteLine(dt.ToString(ciKo));

            CultureInfo ciEn = new CultureInfo("en-US");
            WriteLine();
            WriteLine(dt.ToString("0:yyyy-MM-dd tt hh:mm:ss (ddd)", ciEn));
            WriteLine(dt.ToString("0:yyyy-MM-dd HH:mm:ss (dddd)", ciEn));
